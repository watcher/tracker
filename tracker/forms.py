...
from tracker.models import Ticket, FollowUp, Queue

class PublicNewTicketForm(forms.Form):
	...
	priority = forms.ChoiceField(label='Priority', choices=Ticket.PRIORITY_CHOICES, initial=3, help_text='Please select the priority carefully. Priorities can and will be changed by members of the staff when receiving your issue.')
	attachment = forms.FileField(label='Attach a file', required=False, help_text='You can attach a file (for instance a screenshot) to clarify your issue.')
	...
	def save(self):
		...
		f = FollowUp(ticket=t, title='Ticket opened via web', public=True, comment='Ticket opened via web.')
		f.save()
